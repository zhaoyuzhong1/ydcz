...
//    }
    ...
    public void addResourceHandlers(ResourceHandlerRegistry registry) {
        ...
        if (osName.toLowerCase().startsWith("win")) {  //如果是Windows系统
            //path = "C://gjimage";
            registry.addResourceHandler("/img_car/**")
                    // /apple/**表示在磁盘apple目录下的所有资源会被解析为以下的路径
                    .addResourceLocations("file:D://ydcz_upload//car//") //媒体资源
                    .addResourceLocations("classpath:/WEB-INF/gjimage/");  //swagger2页面

            registry.addResourceHandler("/img_banner/**")
                    // /apple/**表示在磁盘apple目录下的所有资源会被解析为以下的路径
                    .addResourceLocations("file:D://ydcz_upload//banner//") //媒体资源
                    .addResourceLocations("classpath:/WEB-INF/gjimage/");  //swagger2页面
